<template>
  <main>
    <top-show category="FHRSturcture" label="PosterTrial" :info="swiperlists[0]" class="active"> </top-show>
    <div class="breadcrumb">
      <el-breadcrumb separator-class="el-icon-arrow-right">
        <el-breadcrumb-item :to="$path('/')" exact>{{ $ft("home") }}</el-breadcrumb-item>
        <el-breadcrumb-item>{{ $ft("16") }}</el-breadcrumb-item>
      </el-breadcrumb>
    </div>
...
export default {
  ...
  head() {
    return {
      title: this.$ft("z_1"),
      meta: [
        { name: "keywords", content: this.$ft("z_3") },
        { hid: "description", name: "description", content: this.$ft("z_2") }
      ],
      __dangerouslyDisableSanitizers: ["script"],
      script: structuredData("https://www.flash-hr.com/company-organization-structure-management", this.$ft("16"))
    };
  },
  data() {
    return {
